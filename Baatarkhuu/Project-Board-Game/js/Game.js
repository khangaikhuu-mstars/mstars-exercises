class Game {
  ...
  handleKeyDown(event) {
    if (this.ready) {
      if (event.key == "ArrowLeft") {
        ...
        this.playToken();
        // this.activePlayer.activeToken.drop(target, ()=>{console.log(reset)})
      }
    }
  }
  playToken() {
  ...
  // message = "Player 1 Wins, Player 2 Wins, No more tokens avialable"
  gameOver(message) {
  ...
  updateGameState(token, target) {
    ...
    if (!this.checkForWin(target)) {
      console.log("game is continious");
      this.switchPlayer();
      ...
      if (this.activePlayer.checkTokens()) {
        ...
        this.gameOver("No more tokens avialable");
      }
    } else {
